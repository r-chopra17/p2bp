 ...
 */
function p2_theme_ajax_querystring( $query_string, $object ) {
	global $bp;
	...
	return apply_filters( 'p2_theme_ajax_querystring', $query_string, $object, $_BP_COOKIE['bp-' . $object . '-filter'], $_BP_COOKIE['bp-' . $object . '-scope'], $_BP_COOKIE['bp-' . $object . '-page'], $_BP_COOKIE['bp-' . $object . '-search-terms'], $_BP_COOKIE['bp-' . $object . '-extras'] );
}
add_filter( 'bp_ajax_querystring', 'p2_theme_ajax_querystring', 10, 2 );

/* This function will simply load the template loop for the current object. On an AJAX request */
function p2_theme_object_template_loader() {
	$object = esc_attr( $_POST['object'] );
	locate_template( array( "$object/$object-loop.php" ), true );
}
add_action( 'wp_ajax_members_filter', 'p2_theme_object_template_loader' );
add_action( 'wp_ajax_groups_filter', 'p2_theme_object_template_loader' );
add_action( 'wp_ajax_blogs_filter', 'p2_theme_object_template_loader' );
add_action( 'wp_ajax_forums_filter', 'p2_theme_object_template_loader' );

/* This function will load the activity loop template when activity is requested via AJAX */
function p2_theme_activity_template_loader() {
	global $bp;
	...
	$result['feed_url'] = apply_filters( 'p2_theme_activity_feed_url', $feed_url, $_POST['scope'] );
	ob_end_clean();

	echo json_encode( $result );
}
add_action( 'wp_ajax_activity_widget_filter', 'p2_theme_activity_template_loader' );
add_action( 'wp_ajax_activity_get_older_updates', 'p2_theme_activity_template_loader' );

/* AJAX update posting */
function p2_theme_post_update() {
	global $bp;
...
add_action( 'wp_ajax_post_update', 'p2_theme_post_update' );

/* AJAX activity comment posting */
function p2_theme_new_activity_comment() {
	global $bp;
...
add_action( 'wp_ajax_new_activity_comment', 'p2_theme_new_activity_comment' );

/* AJAX delete an activity */
function p2_theme_delete_activity() {
	global $bp;
...
add_action( 'wp_ajax_delete_activity', 'p2_theme_delete_activity' );

/* AJAX delete an activity comment */
function p2_theme_delete_activity_comment() {
	global $bp;
...
add_action( 'wp_ajax_delete_activity_comment', 'p2_theme_delete_activity_comment' );

/* AJAX mark an activity as a favorite */
function p2_theme_mark_activity_favorite() {
	global $bp;

	bp_activity_add_user_favorite( $_POST['id'] );
	_e( 'Remove Favorite', 'buddypress' );
}
add_action( 'wp_ajax_activity_mark_fav', 'p2_theme_mark_activity_favorite' );

/* AJAX mark an activity as not a favorite */
function p2_theme_unmark_activity_favorite() {
	global $bp;

	bp_activity_remove_user_favorite( $_POST['id'] );
	_e( 'Favorite', 'buddypress' );
}
add_action( 'wp_ajax_activity_mark_unfav', 'p2_theme_unmark_activity_favorite' );

/* AJAX invite a friend to a group functionality */
function p2_theme_ajax_invite_user() {
	global $bp;
...
add_action( 'wp_ajax_groups_invite_user', 'p2_theme_ajax_invite_user' );

/* AJAX add/remove a user as a friend when clicking the button */
function p2_theme_ajax_addremove_friend() {
	global $bp;
...
add_action( 'wp_ajax_addremove_friend', 'p2_theme_ajax_addremove_friend' );

/* AJAX accept a user as a friend when clicking the "accept" button */
function p2_theme_ajax_accept_friendship() {
...
add_action( 'wp_ajax_accept_friendship', 'p2_theme_ajax_accept_friendship' );

/* AJAX reject a user as a friend when clicking the "reject" button */
function p2_theme_ajax_reject_friendship() {
...
add_action( 'wp_ajax_reject_friendship', 'p2_theme_ajax_reject_friendship' );

/* AJAX join or leave a group when clicking the "join/leave" button */
function p2_theme_ajax_joinleave_group() {
	global $bp;
...
add_action( 'wp_ajax_joinleave_group', 'p2_theme_ajax_joinleave_group' );

/* AJAX close and keep closed site wide notices from an admin in the sidebar */
function p2_theme_ajax_close_notice() {
	global $userdata;
...
add_action( 'wp_ajax_messages_close_notice', 'p2_theme_ajax_close_notice' );

/* AJAX send a private message reply to a thread */
function p2_theme_ajax_messages_send_reply() {
	global $bp;
...
add_action( 'wp_ajax_messages_send_reply', 'p2_theme_ajax_messages_send_reply' );

/* AJAX mark a private message as unread in your inbox */
function p2_theme_ajax_message_markunread() {
	global $bp;
...
add_action( 'wp_ajax_messages_markunread', 'p2_theme_ajax_message_markunread' );

/* AJAX mark a private message as read in your inbox */
function p2_theme_ajax_message_markread() {
	global $bp;
...
add_action( 'wp_ajax_messages_markread', 'p2_theme_ajax_message_markread' );

/* AJAX delete a private message or array of messages in your inbox */
function p2_theme_ajax_messages_delete() {
	global $bp;
...
add_action( 'wp_ajax_messages_delete', 'p2_theme_ajax_messages_delete' );

/* AJAX autocomplete your friends names on the compose screen */
function p2_theme_ajax_messages_autocomplete_results() {
	global $bp;
...
add_action( 'wp_ajax_messages_autocomplete_results', 'p2_theme_ajax_messages_autocomplete_results' );
