...
$.Autocompleter = function(input, options) {
  ...
  $input.bind(($.browser.opera ? "keypress" : "keydown") + ".autocomplete", function(event) {
    ...
    switch(k) {
      ...
      case KEY.RIGHT:
      case KEY.SEMIC:
        if ( k == KEY.RETURN && select.visible() )
          event.preventDefault();
        if( selectCurrent() ) {
          // stop default to prevent a form submit, Opera needs special handling
          log.debug("in k = " + k + ' options.supressReturn =' + options.supressReturn)
          if (k == KEY.RETURN && options.supressReturn && select.visible() )
            event.preventDefault();
          blockSubmit = true;
          hideResultsNow();
...
$.Autocompleter.defaults = {
  inputClass: "acedit_input",
  resultsClass: "acedit_results",
  loadingClass: "acedit_loading",
  minChars: 1,
  live:false,
...
$.Autocompleter.Select = function (options, input, select, config) {
  ...
  function fillList(q) {
    ...
    for (var i=0; i < max; i++) {
      ...
      if ( formatted === false )
        continue;
      var li = $("<li/>").html( options.highlight(formatted, term) ).addClass(i%2 == 0 ? "acedit_even" : "acedit_odd").appendTo(list)[0];
      $.data(li, "acedit_data", data[i]);
    }
    ...
    if (options.startmsg && (max == 0 || max == undefined) && q == null) {
      var li = $("<li/>").html( options.startmsg )
        .click(function(){
          $(input).trigger("acedit_start_msg_click");
    ...
    if (options.noresultsmsg && (max == 0 || max == undefined) && q != null) {
      ...
      var li = $("<li/>").html( val_noresult )
        .click(function(){
          $(input).trigger("acedit_noresult_click",q);
    ...
    if (options.endmsg && (max != undefined && max > 0)) {
      var li = $("<li/>").html( options.endmsg )
        .click(function(){
          $(input).trigger("acedit_end_message_click",q);
  ...
  return {
    ...
    selected: function() {
      ...
      return selected && selected.length && $.data(selected[0], "acedit_data");
    },
    emptyList: function (){
      list && list.empty();
    },
